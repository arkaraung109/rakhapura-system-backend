...
public class ExamTitleController {
    ...
    @GetMapping("/segment")
    public PaginationResponse<ExamTitleDto> findEachPageSortById(@RequestParam int page, @RequestParam(required = false) String order) {
        boolean isAscending = true;
        if(order!=null && order.equals("desc")) {
            isAscending = false;
        }
        return this.examTitleService.findEachPageSortById(page, isAscending);
    }
    ...
    public ResponseEntity<CustomHttpResponse> save(@Valid @RequestBody ExamTitleDto body, BindingResult bindingResult) {
    ...
    @RolesAllowed(AuthoritiesConstants.EXAM_ENTRY)
    public ResponseEntity<CustomHttpResponse> update(@Validated @RequestBody ExamTitleDto body, @PathVariable("id") Long id, BindingResult bindingResult) {
        if(!bindingResult.hasErrors()) {
